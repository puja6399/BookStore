...
const PostDetails = () => {
  ...
  const deletePost = async () => {
    try {
      const respose = await fetch(`https://bookstore-nl9p.onrender.com/post/${id}`, {
        method: "POST",
        headers: {
  ...
  const fetchPosts = async () => {
    setLoading(true);
    try {
      const response = await fetch(`https://bookstore-nl9p.onrender.com/post/${id}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleBuydata = async (e, sellerEmail) => {
    ...
    if (currentUser && sellerEmail && id) {
      try {
        const response = await fetch("https://bookstore-nl9p.onrender.com/order", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
